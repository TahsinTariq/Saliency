...
def main():
    parser = argparse.ArgumentParser(
        description="AMNet Image memorability prediction with attention"
    )
    parser.add_argument(
        "--gpu",
        default=0,
        type=int,
        help="GPU ID. If -1 the application will run on CPU",
    )
    parser.add_argument(
        "--model-weights",
        default="",
        # default="data/lgcn_2_bs_8_train_1/weights_15.pkl",
        type=str,
        help="pkl file with the model weights",
    )
    ...
    parser.add_argument(
        ...
        default="ResNet50FC",
        type=str,
        help="Name of CNN model for features extraction [ResNet18FC, ResNet50FC, ResNet101FC, VGG16FC, ResNet50FT]",
    )
    parser.add_argument(
        "--att-off",
        action="store_true",
        help="Runs training/testing without the visual attention",
    )
    parser.add_argument(
        "--lstm-steps",
        default=3,
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Directory or a csv file with images to predict memorability for",
    )
    parser.add_argument(
        "--csv-out",
        default="",
        type=str,
        help="File where to save prediced memorabilities in csv format",
    )
    parser.add_argument(
        "--att-maps-out",
        default="",
        type=str,
        help="Directory where to store attention maps",
    )
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Experiment name. Usually no need to set",
    )
    parser.add_argument(
        "--dataset-root", default="", type=str, help="Dataset root directory"
    )
    parser.add_argument(
        "--images-dir",
        default="images",
        type=str,
        help="Relative path to the test/train images",
    )
    parser.add_argument(
        "--splits-dir",
        default="splits",
        type=str,
        help="Relative path to directory with split files",
    )
    parser.add_argument(
        "--train-split",
        # default="",
    ...
    if hps.eval_images != "":
        if hps.model_weights == "" or not os.path.isfile(hps.model_weights):
            print(
                "You need to specify path to model weights with parameter --model-weights"
            )
            return
