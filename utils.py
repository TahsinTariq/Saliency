...
try:
    ...
    print(
        "WARNING: Could not load OpenCV python package. Some functionality may not be available."
    )
...
def get_video_list(video_path, max_len=-1):
    return list_files(
        video_path, extensions=["avi", "flv", "mpg", "mp4"], sort=True, max_len=max_len
    )


def get_image_list(video_path, max_len=-1):
    return list_files(
        video_path, extensions=["jpg", "jpeg", "png"], sort=True, max_len=max_len
    )
...
def get_weight_files(split_files, experiment_name, max_rc_checkpoints=True):
    ...
    for split_filename in split_files:
        ...
        weight_files_all = os.path.join(
            data_dir, experiment_name + "_train_" + split_id + "/*.pkl"
        )
        ...
        if len(files) == 0:
            ...
            if max_rc_checkpoints:
                weights_dir = os.path.join(
                    data_dir, experiment_name + "_train_" + split_id
                )
                print(
                    "Selecting model weights with the highest RC on validation set in ",
                    weights_dir,
                )
                weight_file, max_rc, max_mse, max_epoch = get_max_rc_weights(
                    weights_dir
                )
...
if __name__ == "__main__":
    ...
    weight_files = get_weight_files(
        split_files,
        experiment_name="lamem_ResNet50FC_lstm3_last",
