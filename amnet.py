...
class PredictionResult:
    ...
    def write_stdout(self):
        ...
        for i, (image, prediction, target) in enumerate(
            zip(self.image_names, self.predictions, self.targets)
        ):
            print(
                str(i)
                + "\t"
    ...
    def write_csv(self, filename):
        with open(filename, "wt") as f:
            for image, prediction, target in zip(
                self.image_names, self.predictions, self.targets
            ):
    ...
    def get_attention_maps(self, show=False):
        ...
        for b in range(num_images):
            ...
            offset = 20
            canvas = np.zeros(
                (224 + offset * 2 + 50, (224 + offset * 2) * (seq_len + 1), 3),
                dtype=np.uint8,
            )
            ...
            for s in range(seq_len):
                ...
                heat_map_img = cv2.resize(
                    img_alpha, out_size, interpolation=cv2.INTER_CUBIC
                )
                heat_map_img = cv2.applyColorMap(heat_map_img, cv2.COLORMAP_JET)

                alpha = 0.5
                beta = 1.0 - alpha
                img_heat_map_blend = cv2.addWeighted(
                    img, alpha, heat_map_img, beta, 0.0
                )
                # amaps.append(img_heat_map_blend)

                y_pos = (s + 1) * (224 + offset * 2)
                canvas[
                    offset : 224 + offset, y_pos + offset : y_pos + 224 + offset, :
                ] = img_heat_map_blend
    ...
    def __str__(self):
        ...
        result += (
            ...
            + ("NA" if self.rc is None else str(self.rc))
            + "\n"
        )
        result += "MSE: " + ("NA" if self.mse is None else str(self.mse)) + "\n"
        result += (
            "Inference took: " + str(self.inference_took * 1000000.0) + " us/per image"
        )
        return result
...
class AMNet:
    ...
    def init_transformations(self):
        ...
        if (
            self.hps.torchvision_version_major == 0
            and self.hps.torchvision_version_minor < 2
        ):
    ...
    def load_dataset(
        ...
        dataset = LaMem2(
            os.path.join(dataset_root, self.hps.images_dir),
    ...
    def postprocess(self, output, outputs):
        ...
        if self.hps.last_step_prediction:
            ...
            outputs = (
                outputs / (outputs.shape[1] * self.hps.target_scale)
            ) + self.hps.target_mean / outputs.shape[1]
    ...
    def get_losses(self, output, outputs, alphas, target, criterion):
        ...
        if output is not None:
            reg_loss = criterion(
                output, (target - self.hps.target_mean) * self.hps.target_scale
            )
    ...
    def train_epoch(self, epoch, train_loader):
        ...
        if new_lr != self.lr:
            ...
            self.optimizer = torch.optim.Adam(
                parameters, lr=self.lr, weight_decay=params.l2_req
            )
        ...
        for batch_idx, (data, target, _) in enumerate(train_loader):
            ...
            reg_loss, att_loss, mem_loc_loss = self.get_losses(
                output, outputs, alphas, target, criterion
            )
            ...
            if batch_idx % 10 == 0:
                ...
                print(
                    "{:03d}:{:06.3f} - {} [{: 5d}/{: 5d} ({:.0f}%)]  \tLoss: {:.6f} ({:.6f},  {:.6f})  [{:.3f} sec]".format(
                        ...
                        loss.data.item(),
                        reg_loss.data.item(),
                        (att_loss.data.item() if att_loss is not None else 0),
                        took,
                    )
                )
        ...
        rc, mse = train_loader.dataset.getRankCorrelationWithMSE(
            predictions, gt=targets
        )
        took_total = time.time() - self.total_time
        took_total_min = int(took_total // 60)
        took_total = took_total % 60
        ...
        print(
            "{:03d}:{:06.3f} - {} RC: {:.6f}  MSE: {:.6f}  [{:.3f} sec]".format(
    ...
    def train(self):
        ...
        self.logger.open(
            os.path.join(
                ...
                "train_log_" + str(self.hps.epoch_start) + ".csv",
            )
        )
        self.load_checkpoint(
            os.path.join(
                self.get_experiment_path(),
                "weights_" + str(self.hps.epoch_start) + ".pkl",
            )
        )

        train_data_loader = self.load_dataset(
            split=self.hps.train_split,
            train=True,
    ...
    def eval_model(self, test_loader):
        ...
        print(
            "\nValidation: avg_loss: {:.4f} ({:.4f},  {:.4f})    RC: {:.6f}  MSE: {:.6f}  image_inference: {:.3f} us\n".format(
                ...
                rc,
                mse,
                img_inference_took_avg * 1000000.0,
            )
        )
    ...
    def predict(self, test_loader):
        ...
        if test_loader.dataset.valid_labels:
            rc, mse = test_loader.dataset.getRankCorrelationWithMSE(
                predictions, gt=targets
            )
        else:
            rc, mse = None, None
    ...
    def predict_memorability(self, images_path):
        ...
        dataset = LaMem2(
            split=images_path,  # Load all images if the split points to a directory otherwise expects
        ...
        if len(dataset.data) < batch_size:
            batch_size = len(dataset.data)
            print(
                "Reducing batch size from ", self.hps.test_batch_size, "to", batch_size
            )
